...
public class WorldHandlerClonedStructure implements IWorldGenHandler {
   ...
   public boolean generate(World world, Random random, int x, int z) {
      if (Config.instance().generateGoblinHuts && random.nextDouble() < this.chance) {
         ...
         try {
            Constructor ctor = this.clazz.getConstructor(Integer.TYPE, Random.class, Integer.TYPE, Integer.TYPE, Integer.TYPE, Integer.TYPE, Integer.TYPE);
            ComponentClonedStructure component = (ComponentClonedStructure)ctor.newInstance(direction, random, x, z, this.width, this.height, this.depth);
