...
public class RiteCrater extends Rite {
   ...
   private static class StepCrater extends RitualStep {
         ...
         if (ticks % 10L != 0L) {
            ...
            if (this.stage > height) {
               ...
               for(int y = 1; y <= this.stage; ++y) {
                  ...
                  Log.instance().debug(String.format("Stage: %d, r=%f y=%d", this.stage, r, y));
      ...
      protected void drawPixel(World world, int x, int z, int y, int midX, int midY, int midZ) {
         if (!world.field_72995_K && (x != midX || z != midZ) && (y < midY - 3 || Coord.distance((double)x, (double)midY, (double)z, (double)midX, (double)midY, (double)midZ) > (double)(this.rite.radius - 3 - (midY - y)))) {
