   ...
   private static class StepCallCreatures extends RitualStep {
      ...
      private void allure(World world, double posX, double posY, double posZ, int quad) {
         try {
            ...
            int count = 0;
            int minDistanceSq = true;
            Iterator i$ = world.func_72872_a(EntityCreature.class, bounds).iterator();

            while(i$.hasNext()) {
               Object obj = i$.next();
               EntityCreature creature = (EntityCreature)obj;
               if (this.rite.creatureTypes.contains(creature.getClass()) && creature.func_70092_e(posX, posY, posZ) > 32.0D && !PotionEnderInhibition.isActive(creature, 0)) {
                  ItemGeneral var10000 = Witchery.Items.GENERIC;
                  ItemGeneral.teleportToLocation(world, posX - 2.0D + (double)world.field_73012_v.nextInt(5), posY, posZ - 2.0D + (double)world.field_73012_v.nextInt(5), world.field_73011_w.field_76574_g, creature, true);
                  ++count;
                  if (count >= 2) {
                     break;
                  }
            ...
            Log.instance().debug(String.format("Exception occurred alluring with a ritual! %s", var17.toString()));
         }
