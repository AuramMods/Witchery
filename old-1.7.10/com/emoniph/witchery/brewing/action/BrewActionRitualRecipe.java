...
public class BrewActionRitualRecipe extends BrewAction {
   ...
   private final List<BrewActionRitualRecipe.Recipe> expandedRecipes = new ArrayList();
   ...
   public List<BrewActionRitualRecipe.Recipe> getExpandedRecipes() {
   ...
   public final RitualStatus updateRitual(MinecraftServer server, BrewActionList actionList, World world, int x, int y, int z, ModifiersRitual modifiers, ModifiersImpact impactModifiers) {
      ...
      if (result != null) {
         EntityItem item = new EntityItem(world, (double)x + 0.5D, (double)y + 1.5D, (double)z + 0.5D, result.func_77946_l());
         item.field_70159_w = 0.0D;
         item.field_70181_x = 0.2D;
   ...
   private void removeFromNeededItems(ArrayList<ItemStack> neededItems, BrewItemKey item) {
      ...
      } while(stack.func_77973_b() != item.ITEM || stack.func_77960_j() != item.DAMAGE);
