...
public class BrewActionRitual extends BrewAction {
   ...
   public RitualStatus updateRitual(MinecraftServer server, BrewActionList actionList, World world, int x, int y, int z, ModifiersRitual modifiers, ModifiersImpact impactModifiers) {
      ...
      if (!isDistanceAllowed(world, x, y, z, (double)target.x, (double)target.y, (double)target.z, target.dimension, modifiers.covenSize, modifiers.leonard)) {
         return RitualStatus.FAILED_DISTANCE;
      } else {
         return !actionList.isTargetLocationValid(server, world, x, y, z, target, modifiers) ? RitualStatus.FAILED_INVALID_CIRCLES : impactModifiers.getDispersal().onUpdateRitual(world, x, y, z, actionList.getTagCompound(), modifiers, impactModifiers);
      }
   }
