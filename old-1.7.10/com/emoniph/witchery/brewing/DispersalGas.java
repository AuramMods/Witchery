      ...
      (new BlockActionCircle() {
         public void onBlock(World world, int x, int y, int z) {
            ...
            if (coords != null) {
               ...
               ModifiersEffect effectModifiers = new ModifiersEffect(0.0D, 1.0D, false, new EntityPosition(coords), true, modifiers.covenSize, player);
               ...
               List<EntityLivingBase> entities = EntityUtil.getEntitiesInRadius(EntityLivingBase.class, world, (double)coords.x, (double)coords.y, (double)coords.z, 1.5D);
               Iterator i$ = entities.iterator();

               while(i$.hasNext()) {
                  EntityLivingBase entity = (EntityLivingBase)i$.next();
                  effectModifiers = new ModifiersEffect(1.0D, 1.0D, false, new EntityPosition(coords), true, modifiers.covenSize, player);
